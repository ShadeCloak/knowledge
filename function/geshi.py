    ...
    with open(input_file, 'r', encoding='utf-8') as infile, open(output_file, 'w', encoding='utf-8') as outfile:
        for line in infile:
            ...
            for i in range(len(data["selected_questions"])):
                ...
                if isinstance(data["human_InternLM_answers_ann"][i], list):
                    chosen_content = " ".join(data["human_InternLM_answers_ann"][i])
                else:
                    chosen_content = data["human_InternLM_answers_ann"][i]
...
# 调用函数，进行格式转换
convert_to_training_format('/home/rqn/ANAH/anah/random_sample.jsonl', '/home/rqn/ANAH/anah/training_sample.jsonl')
